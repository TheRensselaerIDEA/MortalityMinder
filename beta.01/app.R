init <- function() {
  setwd("~/data/AHRQ_Challenge/MortalityMinder/beta.01/init")
  source("~/data/AHRQ_Challenge/MortalityMinder/beta.01/init/Librarian.R")
  source("~/data/AHRQ_Challenge/MortalityMinder/beta.01/init/Loader_CHR2019.R")
...
ui <- fluidPage(
  # include css
  tags$head(includeCSS("~/data/AHRQ_Challenge/MortalityMinder/beta.01/www/custom.css")),
  tags$head(
  ...
  includeScript(path = "~/data/AHRQ_Challenge/MortalityMinder/beta.01/inst/myscript.js")
)
...
server <- function(input, output){
  ...
  mort.mat.label <- reactive({
    ...
    dplyr::left_join(mort.label, mort.mat, by = "county_fips")
  })
  ...
  output$geo_cluster_mort <- renderPlot({
    ...
    #Fetch geo-data
    geo.data <- geo.map.fetch.dev(input$state_choice, geo.namemap)
