...
Loading the mortality data that will be used in the Kendall correlation analysis below
```{r}
...
```{r}
# csv_factors <- read_csv("init/SocialDeterminants.csv")
# 
# csv_factors_1s <- csv_factors[csv_factors$Keep == 1,]["Code"]
# csv_factors_1s <- append(csv_factors_1s$Code, "county_fips", after = 0)
# 
# print(csv_factors_1s)
```


Loading the actual factors data which was obtained from county health rankings
```{r}
chr.data.2019 <- readRDS("init/chr.data.2019.rds")

#Only keeping the factors that have keep value as 1
chr.data.2019 <- chr.data.2019 %>%
  as_data_frame %>%
  select(csv_factors_1s)
head(chr.data.2019)
```
...
selected_SDs <- intersect(selected_SDs, selected_SDs_allcause$chr_code)
print(selected_SDs)
```
